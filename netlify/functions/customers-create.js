...
module.exports.handler = async (event) => {
  try {
    ...
    if (type === 'piva') { piva = (body.piva||'').trim(); if (!piva) return failure('P.IVA obbligatoria per aziende', 400); }
    else { cf = (body.cf||'').trim(); if (!cf) return failure('Codice Fiscale obbligatorio per privati', 400); }

    return await withClient(async (client) => {
      await ensureSchema(client);
      const id = uuid();
      await client.query(`INSERT INTO customers (id, type, name, email, piva, cf) VALUES ($1,$2,$3,$4,$5,$6)`,
