...
export default function CER() {
  ...
  useEffect(()=>{ (async ()=>{ setBusy(true); setErr(''); try { await Promise.all([loadCER(), loadCustomers()]); } catch { setErr('Errore nel caricamento'); } finally { setBusy(false); } })(); }, []);

  const toggleMember = (cust: Customer, role:'producer'|'consumer') => {
    setMembers(prev => {
      const i = prev.findIndex(m => m.customer_id === cust.id);
      if (i === -1) return [...prev, { customer_id: cust.id, role }];
      // se già presente, aggiorna il ruolo
      const copy = [...prev];
      copy[i] = { customer_id: cust.id, role };
      return copy;
    });
  };
  ...
  const submit = async (e:React.FormEvent) => {
    ...
    try {
      await apiPost('cer-create', {
        ...
        quota_shared: n(f.quota_shared),
        split_prod: calc.prod,
        // prosumer calcolato lato server, ma mando quello di calcolo locale per coerenza
      ...
      const msg = e?.message || 'Errore nel salvataggio CER';
      setErr(msg);
    } finally { setBusy(false); }
  };
